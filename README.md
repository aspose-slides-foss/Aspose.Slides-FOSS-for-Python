...
- Open, create, and save presentations through `Presentation`, always as a context manager
  ...
  understand.
- Add, remove, clone, hide, and iterate slides through `SlideCollection` (`add_empty_slide()`,
  `remove_at()`, `add_clone()`) and `Slide` / `LayoutSlide` / `MasterSlide`, including per-slide
  visibility (`slide.hidden`) and layout/master relationships.
...
- Attach an external hyperlink, with an optional tooltip, to a single text portion
  (`portion_format.hyperlink_click`) or to a whole shape, including a "mouse over" variant
  (`shape.hyperlink_mouse_over`), through `Hyperlink`; assigning `None` removes the link and the
  relationship it owns. See [Installation](#installation) — this needs the from-source install.
...
- Read and write presentation metadata with `DocumentProperties` (core, app, and custom
  ...
  manage classic and modern threaded comments — authors, timestamps, replies, and positions — with
  `CommentAuthorCollection` / `CommentCollection` (see
  [Scope and Limitations](#scope-and-limitations) for what writing a new reply preserves).
- Embed images from a file, bytes, or stream with `Images.from_file()` / `Images.from_stream()`,
  or add one directly to a slide with `ImageCollection.add_image()`, which accepts image bytes or
  a file-like object — not a file path; open the file yourself and pass the handle or its bytes.
...
**The PyPI release predates the source tree behind this page.** `pip install` gets `26.8.0`, and
the changes listed in [CHANGELOG.md](CHANGELOG.md) under `## [Unreleased]` — including hyperlinks,
which several examples on this page use, and the six distinct PowerPoint save formats — are not in
that release. For the behaviour described here, install from the repository instead:

```bash
pip install git+https://github.com/aspose-slides-foss/Aspose.Slides-FOSS-for-Python.git
```

**`aspose` is a shared namespace package.** If the commercial `aspose` package is installed in the
same environment, it takes the name over and this library becomes unimportable, with a bare
`ModuleNotFoundError: No module named 'aspose.slides_foss'`. Install this one into its own virtual
environment.
...
`add_image` takes the image bytes, or any object with a `.read()` method — not a file path. Open
the file yourself, which keeps the library out of the business of guessing encodings and leaving
handles open:
...
- **[Publishing guide](PUBLISHING.md)** — how a release is cut and published to PyPI.

## Scope and Limitations

The following are not implemented, and the API member is absent rather than present and inert:

- **Rendering and conversion of any kind** — no PDF, HTML, XPS, or image export. This is a design
  boundary, not a gap: the library writes OOXML, it does not lay out or rasterize.
...
- **Slide size**: there is no public API to read or change it, so a new deck is whatever the
  bundled template says — 16:9, `12192000 × 6858000` EMU.
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs that same suite across the full
Python/OS matrix on every push and pull request; releases publish to PyPI through
[`.github/workflows/publish.yml`](.github/workflows/publish.yml).

See [`agents.md`](agents.md) for the package's internal structure and contribution conventions, and
[CONTRIBUTING.md](CONTRIBUTING.md)
(https://github.com/aspose-slides-foss/Aspose.Slides-FOSS-for-Python/blob/main/CONTRIBUTING.md)
for how to build, how to run the tests, and how to report a fix. What changed, and what changed in
a way you will notice, is recorded in
[CHANGELOG.md](https://github.com/aspose-slides-foss/Aspose.Slides-FOSS-for-Python/blob/main/CHANGELOG.md);
the security policy for how to report a vulnerability privately is
[SECURITY.md](https://github.com/aspose-slides-foss/Aspose.Slides-FOSS-for-Python/blob/main/SECURITY.md)
rather than a public issue; participation is governed by
[CODE_OF_CONDUCT.md](https://github.com/aspose-slides-foss/Aspose.Slides-FOSS-for-Python/blob/main/CODE_OF_CONDUCT.md).
